package com.dercg.leetcode.code.a105_ConstructBinaryTreeFromPreorderAndInorderTraversal;
...
public class main {
    static class Solution {
        ...
        public TreeNode build(int[] preorder, int[] inorder, int preLeft, int preRight, int inLeft, int inRight) {
            ...
            int pIndex = indexMap.get(root.val);

            TreeNode left = build(preorder, inorder, preLeft + 1, pIndex - inLeft + preLeft, inLeft, pIndex - 1);
            TreeNode right = build(preorder, inorder, pIndex - inLeft + preLeft + 1, preRight, pIndex + 1, inRight);
